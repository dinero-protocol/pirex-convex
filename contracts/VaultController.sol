...
contract VaultController is Ownable {
    ...
    event CreatedRewardCvxVault(address vault, uint256 epoch, string tokenId);
    event CreatedVotiumRewardClaimer(
        address votiumRewardClaimer,
        address lockedCvxVault,
        address[8] rewardCvxVaults
    );
    event SetUpVaults(
        address lockedCvxVault,
        address[8] rewardCvxVaults,
    ...
    constructor(
        ...
        REWARD_CVX_VAULT_IMPLEMENTATION = address(new RewardCvxVault());
    ...
    /**
        @notice Create a RewardCvxVault
        @param   epoch    uint256         Epoch
        @return  address  RewardCvxVault  address
     */
    function _createRewardCvxVault(uint256 epoch) internal returns (address) {
        if (rewardCvxVaultsByEpoch[epoch] != address(0)) revert AlreadyExists();

        RewardCvxVault vault = RewardCvxVault(
            Clones.clone(REWARD_CVX_VAULT_IMPLEMENTATION)
        );
        string memory tokenId = string(
            abi.encodePacked("rewardCVX-", epoch.toString())
        );
        address vaultAddr = address(vault);
        rewardCvxVaultsByEpoch[epoch] = vaultAddr;

        vault.initialize(address(this), epoch, tokenId, tokenId);

        emit CreatedRewardCvxVault(vaultAddr, epoch, tokenId);
    ...
    /**
        ...
        @param  rewardCvxVaults  address[8]  RewardCvxVault addresses
        @param  tokenEpochs      uint256[8]  RewardCvxVault epochs
     */
    function _createVotiumRewardClaimer(
        ...
        unchecked {
            for (uint8 i; i < 8; ++i) {
                RewardCvxVault(rewardCvxVaults[i]).setRewardClaimer(vAddr);
            }
        }

        emit CreatedVotiumRewardClaimer(vAddr, lockedCvxVault, rewardCvxVaults);
    ...
    /**
        ...
        @return  rewardCvxVaults        address[8]  RewardCvxVault addresses
    ...
    */
        ...
        )
    {
        ...
        // Use the next epoch as a starting point for RewardCvxVaults since voting
        ...
        // Create a RewardCvxVault for each Convex voting round (8 total per lock)
        for (uint8 i; i < 8; ++i) {
            tokenEpochs[i] = startingEpoch + (i * EPOCH_DEPOSIT_DURATION);

            rewardCvxVaults[i] = rewardCvxVaultsByEpoch[tokenEpochs[i]] ==
                address(0)
                ? _createRewardCvxVault(tokenEpochs[i])
                : rewardCvxVaultsByEpoch[tokenEpochs[i]];
        }
        ...
        emit SetUpVaults(lockedCvxVault, rewardCvxVaults, votiumRewardClaimer);
    }

    /**
        @notice Mint rewardCVX tokens
    ...
    */
    ...
    function deposit(address to, uint256 amount) external {
        ...
        v.deposit(to, amount);
        _mintRewardCvxTokens(currentEpoch + EPOCH_DEPOSIT_DURATION, to, amount);
