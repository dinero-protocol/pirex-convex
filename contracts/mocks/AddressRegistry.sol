...
contract AddressRegistry is Ownable {
    ...
    function setRegistry(address _to) public {
        ...
        require(
            registry[msg.sender].start == 0 ||
                registry[msg.sender].expiration <= current,
    ...
    function setToExpire() public {
        ...
        require(
            ...
            "Not registered or expiration already pending"
        );
        ...
        returns (address[] memory)
    {
        ...
        for (uint256 i = 0; i < accounts.length; i++) {
            ...
            if (
                registry[accounts[i]].start <= current &&
                registry[accounts[i]].start != 0 &&
                registry[accounts[i]].expiration > current
            ) {
        ...
        returns (address[] memory)
    {
        ...
        for (uint256 i = page; i < optOutHistory.length; i++) {
            if (
                registry[optOutHistory[i]].start <= current &&
                registry[optOutHistory[i]].expiration > current &&
                registry[optOutHistory[i]].to == address(0)
            ) {
                n++;
                if (n == size) {
                    break;
        ...
        returns (address[] memory)
    {
        ...
        for (uint256 i = page; i < forwardHistory.length; i++) {
            if (
                registry[forwardHistory[i]].start <= current &&
                registry[forwardHistory[i]].expiration > current &&
                registry[forwardHistory[i]].to != address(0)
            ) {
                n++;
                if (n == size) {
                    break;
    ...
    function forceRegistry(address _from, address _to) public onlyOwner {
        uint256 current = currentEpoch();
        require(
            registry[_from].start == 0 || registry[_from].expiration < current,
            "Registration is still active"
        );
    ...
    function forceToExpire(address _from) public onlyOwner {
        uint256 next = nextEpoch();
        require(
            registry[_from].start > 0 && registry[_from].expiration > next,
            "Not registered or expiration already pending"
        );
        // if not started yet, nullify instead of setting expiration
