...
// https://etherscan.io/address/0xe096ccec4a1d36f191189fe61e803d8b2044dfc3#code
...
contract CvxStakingProxy {
    ...
    function setApprovals() external {
        ...
        IERC20(cvx).safeApprove(cvxStaking, 2**256 - 1);

        IERC20(crv).safeApprove(crvDeposit, 0);
        IERC20(crv).safeApprove(crvDeposit, 2**256 - 1);

        IERC20(cvxCrv).safeApprove(rewards, 0);
        IERC20(cvxCrv).safeApprove(rewards, 2**256 - 1);
    }
    ...
    function distribute() external {
        ...
        if (cvxCrvBal > 0) {
            uint256 incentiveAmount = cvxCrvBal.mul(callIncentive).div(
                denominator
            );
    ...
    function distributeOther(IERC20 _token) external {
        ...
        if (bal > 0) {
            ...
            _token.safeApprove(rewards, 2**256 - 1);
