...
contract MultiMerkleStash is Ownable {
        ...
        returns (bool)
    {
        ...
        uint256 claimedWord = claimedBitMap[token][update[token]][
            claimedWordIndex
        ];
    ...
    function claim(
        ...
        require(
            MerkleProof.verify(merkleProof, merkleRoot[token], node),
            "Invalid proof."
        );
    ...
    function claimMulti(address account, claimParam[] calldata claims)
        external
    {
    ...
    function updateMerkleRoot(address token, bytes32 _merkleRoot)
        public
        onlyOwner
    {
